...
export class AuthService {
    ...
    private connectedAdherent: 'connectedAdherent';
    private connectedModerateur: 'connecteModerateur';
    ...
    constructor(private http: HttpClient, private tokenService: TokenService, private router: Router) { }
       ...
       public loginModerateur(username: string, password: string) {
       this.http.post<any>(this.API + 'login', { username, password }, { observe: 'response' }).subscribe(
            resp => {
                this.error = null;
                const jwt = resp.headers.get('Authorization');
                ...
                this.loadInfos();
                console.log('you are logged in successfully');
                this.router.navigate(['/' + environment.rootAppUrl + '/moderateur']);
            }, (error: HttpErrorResponse) => {
                this.error = error.error;
                console.log(error);
            }
       ...
       this.http.post<any>(this.API + 'login', { username, password }, { observe: 'response' }).subscribe(
            resp => {
                ...
                this.loadInfos();
                console.log('you are logged in successfully');
                this.router.navigate(['/' + environment.rootAppUrl + '/adherent']);
            }, (error: HttpErrorResponse) => {
                this.error = error.error;
                console.log(error);
            }
    ...
    public registerModerateur() {
        console.log(this.user);
        this.http.post<any>(this.API + 'api/users/save', this.user, {observe: 'response'}).subscribe(
            resp => {
                this.router.navigate(['moderateur/login']);
    ...
    public registerAdherent() {
        console.log(this.user);
        this.http.post<any>(this.API + 'api/users/save', this.user, {observe: 'response'}).subscribe(
            resp => {
                this.router.navigate(['adherent/login']);
    ...
    }else if (localStorage.getItem(this.connectedModerateur) !== null){
        return JSON.parse(localStorage.getItem(this.connectedModerateur));
    }else if (localStorage.getItem(this.connectedAdherent) !== null){
        return JSON.parse(localStorage.getItem(this.connectedAdherent));
    }
    return null;
    }
    ...
    public registerConnectedModerateur(moderateur: ModerateurVo): void{
    localStorage.setItem(this.connectedModerateur, JSON.stringify(moderateur));
    }
    public unregisterConnectedModerateur(): void{
    localStorage.removeItem(this.connectedModerateur);
    }

    public registerConnectedAdherent(adherent: AdherentVo): void{
    localStorage.setItem(this.connectedAdherent, JSON.stringify(adherent));
    }
    public unregisterConnectedAdherent(): void{
    localStorage.removeItem(this.connectedAdherent);
    }
}

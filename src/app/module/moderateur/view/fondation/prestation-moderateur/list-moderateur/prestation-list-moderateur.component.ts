...
@Component({
  selector: 'app-prestation-list-moderateur',
  templateUrl: './prestation-list-moderateur.component.html',
  styleUrls: ['./prestation-list-moderateur.component.css']
})
export class PrestationListModerateurComponent implements OnInit {
   // declarations
...
public async archiverPrestation(prestation:PrestationVo){
const isPermistted = await this.roleService.isPermitted('Prestation', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Prestation) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.prestationService.archiver(prestation).subscribe(status=>{
const myIndex = this.prestations.indexOf(prestation);
this.prestations[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Prestation archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverPrestation(prestation:PrestationVo){
const isPermistted = await this.roleService.isPermitted('Prestation', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Prestation) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.prestationService.desarchiver(prestation).subscribe(status=>{
const myIndex = this.prestations.indexOf(prestation);
this.prestations[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Prestation désarchivé',
life: 3000
});
},error=>console.log(error))
 }

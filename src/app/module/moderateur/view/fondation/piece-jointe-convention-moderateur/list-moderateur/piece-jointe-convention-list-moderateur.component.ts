...
@Component({
  selector: 'app-piece-jointe-convention-list-moderateur',
  templateUrl: './piece-jointe-convention-list-moderateur.component.html',
  styleUrls: ['./piece-jointe-convention-list-moderateur.component.css']
})
export class PieceJointeConventionListModerateurComponent implements OnInit {
   // declarations
...
public async archiverPieceJointeConvention(pieceJointeConvention:PieceJointeConventionVo){
const isPermistted = await this.roleService.isPermitted('PieceJointeConvention', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Piece jointe convention) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.pieceJointeConventionService.archiver(pieceJointeConvention).subscribe(status=>{
const myIndex = this.pieceJointeConventions.indexOf(pieceJointeConvention);
this.pieceJointeConventions[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Piece jointe convention archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverPieceJointeConvention(pieceJointeConvention:PieceJointeConventionVo){
const isPermistted = await this.roleService.isPermitted('PieceJointeConvention', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Piece jointe convention) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.pieceJointeConventionService.desarchiver(pieceJointeConvention).subscribe(status=>{
const myIndex = this.pieceJointeConventions.indexOf(pieceJointeConvention);
this.pieceJointeConventions[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Piece jointe convention désarchivé',
life: 3000
});
},error=>console.log(error))
 }

...
@Component({
  selector: 'app-mission-list-moderateur',
  templateUrl: './mission-list-moderateur.component.html',
  styleUrls: ['./mission-list-moderateur.component.css']
})
export class MissionListModerateurComponent implements OnInit {
   // declarations
...
public async archiverMission(mission:MissionVo){
const isPermistted = await this.roleService.isPermitted('Mission', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Mission) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.missionService.archiver(mission).subscribe(status=>{
const myIndex = this.missions.indexOf(mission);
this.missions[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Mission archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverMission(mission:MissionVo){
const isPermistted = await this.roleService.isPermitted('Mission', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Mission) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.missionService.desarchiver(mission).subscribe(status=>{
const myIndex = this.missions.indexOf(mission);
this.missions[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Mission désarchivé',
life: 3000
});
},error=>console.log(error))
 }

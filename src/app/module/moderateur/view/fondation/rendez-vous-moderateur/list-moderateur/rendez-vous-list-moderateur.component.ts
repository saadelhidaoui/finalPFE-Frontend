...
@Component({
  selector: 'app-rendez-vous-list-moderateur',
  templateUrl: './rendez-vous-list-moderateur.component.html',
  styleUrls: ['./rendez-vous-list-moderateur.component.css']
})
export class RendezVousListModerateurComponent implements OnInit {
   // declarations
...
public async archiverRendezVous(rendezVous:RendezVousVo){
const isPermistted = await this.roleService.isPermitted('RendezVous', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Rendez vous) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.rendezVousService.archiver(rendezVous).subscribe(status=>{
const myIndex = this.rendezVouss.indexOf(rendezVous);
this.rendezVouss[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Rendez vous archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverRendezVous(rendezVous:RendezVousVo){
const isPermistted = await this.roleService.isPermitted('RendezVous', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Rendez vous) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.rendezVousService.desarchiver(rendezVous).subscribe(status=>{
const myIndex = this.rendezVouss.indexOf(rendezVous);
this.rendezVouss[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Rendez vous désarchivé',
life: 3000
});
},error=>console.log(error))
 }

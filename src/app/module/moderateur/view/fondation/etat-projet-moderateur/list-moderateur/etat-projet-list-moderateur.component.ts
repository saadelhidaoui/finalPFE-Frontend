...
@Component({
  selector: 'app-etat-projet-list-moderateur',
  templateUrl: './etat-projet-list-moderateur.component.html',
  styleUrls: ['./etat-projet-list-moderateur.component.css']
})
export class EtatProjetListModerateurComponent implements OnInit {
   // declarations
...
public async archiverEtatProjet(etatProjet:EtatProjetVo){
const isPermistted = await this.roleService.isPermitted('EtatProjet', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Etat projet) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.etatProjetService.archiver(etatProjet).subscribe(status=>{
const myIndex = this.etatProjets.indexOf(etatProjet);
this.etatProjets[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Etat projet archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverEtatProjet(etatProjet:EtatProjetVo){
const isPermistted = await this.roleService.isPermitted('EtatProjet', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Etat projet) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.etatProjetService.desarchiver(etatProjet).subscribe(status=>{
const myIndex = this.etatProjets.indexOf(etatProjet);
this.etatProjets[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Etat projet désarchivé',
life: 3000
});
},error=>console.log(error))
 }

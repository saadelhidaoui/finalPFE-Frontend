...
@Component({
  selector: 'app-produit-list-moderateur',
  templateUrl: './produit-list-moderateur.component.html',
  styleUrls: ['./produit-list-moderateur.component.css']
})
export class ProduitListModerateurComponent implements OnInit {
   // declarations
         ...
         if(isPermistted){
          this.produitService.findByIdWithAssociatedList(produit).subscribe(res => {
           this.selectedProduit = res;
            this.selectedProduit.dateArrivee = new Date(produit.dateArrivee);
...
public async duplicateProduit(produit: ProduitVo) {
	...
	initDuplicateProduit(res: ProduitVo) {
        if (res.pieceJointeProduitsVo != null) {
             res.pieceJointeProduitsVo.forEach(d => { d.produitVo = null; d.id = null; });
                }
  ...
  initExport(): void {
    ...
    return {
      ...
      this.criteriaData = [{
            ...
            'Date arrivee Min': this.searchProduit.dateArriveeMin ? this.datePipe.transform(this.searchProduit.dateArriveeMin , this.dateFormat) : environment.emptyForExport ,
            'Date arrivee Max': this.searchProduit.dateArriveeMax ? this.datePipe.transform(this.searchProduit.dateArriveeMax , this.dateFormat) : environment.emptyForExport ,
            'Quantite Min': this.searchProduit.quantiteMin ? this.searchProduit.quantiteMin : environment.emptyForExport ,
            'Quantite Max': this.searchProduit.quantiteMax ? this.searchProduit.quantiteMax : environment.emptyForExport ,
            'Piece jointe produits Min': this.searchProduit.pieceJointeProduitsMin ? this.searchProduit.pieceJointeProduitsMin : environment.emptyForExport ,
            'Piece jointe produits Max': this.searchProduit.pieceJointeProduitsMax ? this.searchProduit.pieceJointeProduitsMax : environment.emptyForExport ,
            ...
            'Admin': this.searchProduit.admin ? (this.searchProduit.admin ? environment.trueValue : environment.falseValue) : environment.emptyForExport ,
            'Visible': this.searchProduit.visible ? (this.searchProduit.visible ? environment.trueValue : environment.falseValue) : environment.emptyForExport ,
            'Username': this.searchProduit.username ? this.searchProduit.username : environment.emptyForExport ,
     }];
